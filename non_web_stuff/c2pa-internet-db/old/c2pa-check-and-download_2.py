...
# Uses LIKE on a VARBINARY blob after converting to UTF-8 text and uppercasing.
# We force a binary collation on the normalized text so LIKE semantics stay
# deterministic regardless of connection/session collation.
_METADATA_UPPER_EXPR = "UPPER(CONVERT(i.img_metadata USING utf8mb4)) COLLATE utf8mb4_bin"
...
def _sql_upper_like_pattern(text: str) -> str:
    """Uppercase and escape SQL LIKE wildcards for a literal substring match."""
    upper = _sql_upper_literal(text)
    return upper.replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_")


_like_clauses = "\n    OR ".join(
    f"""{_METADATA_UPPER_EXPR} LIKE '%{_sql_upper_like_pattern(model)}%' ESCAPE '\\\\'"""
