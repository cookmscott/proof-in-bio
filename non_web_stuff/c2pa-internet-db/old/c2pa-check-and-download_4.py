...
REQUEST_DELAY = float(os.environ.get("REQUEST_DELAY", 8.0))
SEED_CHUNK_SIZE = 500_000
PROBE_BATCH_SIZE = 100
PROBE_LIMIT = int(os.environ.get("C2PA_PROBE_LIMIT", 0)) or None
RUN_ID = os.environ.get("C2PA_RUN_ID", uuid.uuid4().hex[:12])
...
CREATE TABLE IF NOT EXISTS c2pa_check_results (
    ...
    claim_run_id      VARCHAR(64)     NULL DEFAULT NULL,
    created_at        TIMESTAMP       DEFAULT CURRENT_TIMESTAMP,
    checked_at        TIMESTAMP       NULL DEFAULT NULL,
    UNIQUE KEY uq_img_name     (img_name),
    KEY idx_pending            (has_c2pa_probe, claimed_at),
...
UPDATE c2pa_check_results
   SET claimed_at = NOW(), claim_run_id = %s
 WHERE has_c2pa_probe IS NULL
   AND (claimed_at IS NULL OR claimed_at < NOW() - INTERVAL %s MINUTE)
 ORDER BY id
 LIMIT %s
"""
...
SELECT img_name, img_size, img_width, img_height, camera_make, camera_model
 ...
 WHERE claim_run_id = %s AND has_c2pa_probe IS NULL
 ORDER BY id
"""
...
def run_seed(conn, seed_limit: int | None = None):
    ...
    log.info(
        "Seed: scanning image table for "
        f"{len(C2PA_CAMERAS)} camera models in one pass"
    ...
    while True:
        if seed_limit is not None and total_matched >= seed_limit:
            break

        chunk_num += 1

        with conn.cursor() as cur:
            cur.execute(SEED_CHUNK_SQL, [last_name] + SEED_LIKE_PARAMS + [SEED_CHUNK_SIZE])
            rows = cur.fetchall()
        ...
        last_name = rows[-1][0]
        log.info(
            f"  Chunk {chunk_num}: scanned {len(rows)}, "
            f"matched {len(insert_data)} (total matched {total_matched}), "
            f"total seeded {total_seeded}"
        )

    log.info(
        "Seed complete. "
        f"Total matched: {total_matched}, total seeded: {total_seeded}"
    )
...
def process_candidate(cur, img_name: bytes, img_size, img_width, img_height,
    ...
    if probe_data is None:
        update_row(cur, img_name, None, None, None, 0, None, probe_info, "Range request failed")
        return
...
def run_probe(conn):
    ...
    total = 0
    log.info(f"Probe starting. run_id={RUN_ID}, limit={'unlimited' if remaining is None else remaining}")

    while True:
        if remaining is not None and remaining <= 0:
            break

        batch_size = PROBE_BATCH_SIZE if remaining is None else min(PROBE_BATCH_SIZE, remaining)

        with conn.cursor() as cur:
            cur.execute(CLAIM_BATCH, (RUN_ID, CLAIM_TIMEOUT_MINUTES, batch_size))
        ...
        log.info(f"Claimed {claimed} rows")

        with conn.cursor() as cur:
            cur.execute(FETCH_CLAIMED, (RUN_ID,))
        ...
        with conn.cursor() as cur:
            for img_name, img_size, img_width, img_height, camera_make, camera_model in candidates:
                try:
                    ...
                    try:
                        update_row(cur, img_name, None, None, None, 0, None, None, str(e))
                    except Exception:
                        pass
