...
# ---------------------------------------------------------------------------
# C2PA-capable cameras/manufacturers
# ---------------------------------------------------------------------------

C2PA_MANUFACTURERS = ["Leica"]
# C2PA_CAMERAS = ["Pixel 10", "Alpha 1 II", "M11-D", "M11-P", "M EV1", "Q3 Monochrom", "SL3-S"]
C2PA_CAMERAS = ["ILCE-1", "ILCE-1M2", "ILCE-9M3", "ILCE-7M4", "ILCE-7SM3", "ILCE-7RM5"]
...
# Parameterized LIKE patterns for seed query:
# manufacturers + camera models (e.g. '%LEICA%', '%PIXEL 10%', '%ALPHA II%')
SEED_TERMS = C2PA_MANUFACTURERS + C2PA_CAMERAS
SEED_LIKE_PARAMS = [f"%{like_escape(term)}%" for term in SEED_TERMS]
SEED_METADATA_EXPR = "CONVERT(i.img_metadata USING utf8mb4)"
SEED_LIKE_WHERE = " OR ".join([f"{SEED_METADATA_EXPR} LIKE %s"] * len(SEED_TERMS))
...
SELECT
  ...
  i.img_width,
  i.img_height,
  CONVERT(i.img_metadata USING utf8mb4) AS img_metadata_text
FROM image i
 WHERE i.img_timestamp >= _binary'20220101000000'
   AND ({SEED_LIKE_WHERE})
   AND NOT EXISTS (
    SELECT 1
...
def match_camera(metadata_text: str) -> tuple[str | None, str | None] | None:
    """Match metadata text against configured C2PA manufacturers/models."""
    upper = metadata_text.upper()
    matched_make = next((make for make in C2PA_MANUFACTURERS if make.upper() in upper), None)
    matched_model = next((model for model in C2PA_CAMERAS if model.upper() in upper), None)
    if matched_make is None and matched_model is None:
        return None
...
def run_seed(conn, seed_limit: int | None = None):
    ...
    log.info(
        ...
        f"{len(SEED_TERMS)} manufacturers/models"
    ...
    with conn.cursor() as cur:
        ...
        log.info("SEED_CANDIDATES_SQL (resolved):\n%s", cur.statement)
        rows = cur.fetchall()
