...
<div class={cn("flex flex-col min-h-screen justify-center items-center gap-6")}>
	<Card class="overflow-hidden p-0 max-w-3xl w-full">
		<CardContent class="grid p-0 md:grid-cols-2">
			<form method="post" action="?/{isLogin ? 'login' : 'signup'}" class="p-6 md:p-8">
				<div class="flex flex-col gap-6">
					...
					<div class="grid gap-3">
						...
						<Input id="email" name="email" type="email" placeholder="m@example.com" bind:value={email} required class="" />
					</div>
					<div class="grid gap-3">
						...
						<Input id="password" name="password" type="password" bind:value={password} required class="" />
					</div>
					<Button type="submit" class="w-full"> {isLogin ? 'Login' : 'Sign Up'} </Button>
					...
					<div class="grid grid-cols-3 gap-4">
						...
						<Button variant="outline" type="button" class="w-full" on:click={handleGoogleSignIn}>
							<!-- Google SVG -->
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="size-5">
