	...
	<div class="relative">
		...
		<main class="relative z-10 container mx-auto max-w-screen-lg px-4 sm:pt-20 pt-8 pb-8">
					...
					<div class="flex-grow min-w-0">
						...
						>
							...
							{#if showCopiedTooltip}
								<span
									class="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-primary text-primary-foreground text-xs px-2 py-1 rounded-md whitespace-nowrap z-50 animate-in fade-in zoom-in-95 duration-200 shadow-md"
								>
									Copied to clipboard
								</span>
							{/if}
				...
				<div class="mt-4 flex gap-2 md:absolute md:top-0 md:right-0 md:mt-0">
					<ShareDrawer
						...
						description={profile.bio ||
							`Check out ${profile.display_name}'s profile on Proof in Bio`}
						label="Share Profile"
			...
			{#if photos.length === 0}
				{#if canEdit}
					<div
						class="flex flex-col items-center justify-center py-16 text-center border-2 border-dashed rounded-lg border-muted mt-4"
					>
	...
	<div class="fixed bottom-0 left-0 right-0 z-[999] border-t bg-background/95 backdrop-blur-sm">
		<div class="container mx-auto max-w-screen-lg px-4 py-3">
			<div class="flex gap-3">
				<Button variant="ghost" class="w-1/3" onclick={toggleEditImages}>Cancel</Button>
				<Button
					variant="destructive"
...
<style>
	...
	:global(.init-expand-btn) {
		animation: expand-btn 3s ease-out;
	}
	:global(.init-expand-icon) {
		animation: expand-icon 3s ease-out;
	}
