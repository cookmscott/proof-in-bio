...
<div class="bg-background text-foreground min-h-screen">
	<div class="relative">
		...
		<div class="absolute top-0 left-0 h-[300px] w-full hidden dark:block">
			<div
				class="absolute inset-0 bg-gradient-to-t from-background via-background/70 to-background/30"
			></div>
		</div>
		...
		<main class="relative z-10 container mx-auto max-w-screen-lg px-4 pt-20 pb-8">
			<!-- Profile Header -->
			<div
				class="relative z-10 mb-8 flex flex-col items-center gap-4 text-center md:flex-row md:text-left"
			>
			...
			{#if editing && data.canEdit}
				<Card class="p-6 mb-8">
					...
					{#if form?.error}
						<div
							class="mb-4 p-3 bg-destructive/10 border border-destructive rounded-md text-destructive text-sm"
						>
							{form.error}
						</div>
					{/if}
					...
					<form method="POST" action="?/updateProfile" use:enhance enctype="multipart/form-data">
						<div class="space-y-4">
							...
							<div>
								...
								<div class="flex items-center gap-4">
									...
									<div class="flex-1">
										...
										<p class="text-xs text-muted-foreground mt-2">JPG, PNG or WebP. Max 2MB.</p>
									</div>
								</div>
							</div>
							...
							<div>
								<label for="interests" class="block text-sm font-medium mb-1"
						...
						<div class="flex gap-2 mt-6">
							...
							<Button type="button" variant="secondary" onclick={() => (editing = false)}
								>Cancel</Button
							>
						</div>
					</form>
